...
export default async function InvoiceDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [data, settings] = await Promise.all([
    getInvoiceDetail({ id: Number(id), role: profile.role, ownBranchId: profile.branchId }),
    getSettings(),
  ]);
  if (!data) notFound();
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-6">
      ...
      <div className="card p-8 print:border-0 print:p-0">
        {/* BRANDING (Sir, 2026-08-09): this header used to read "⚡ Test Branch
            Lahore" — a placeholder emoji and the BRANCH name, on the one document
            a customer actually takes home. The company is what belongs at the
            top; the branch is the issuing office and belongs beneath it.
        ...
        <p className="mt-10 border-t border-line pt-4 text-center text-xs text-ink-faint">
          {settings.companyName || APP_NAME} · {d(new Date())}
        </p>
      </div>
    </div>
  );
