  ...
  return (
    <div className="mx-auto max-w-4xl space-y-6">
      <div className="flex items-center justify-between">
        <Link href="/customers" className="text-sm text-slate-500 hover:text-slate-800">← Back to Customers</Link>
      </div>
      ...
      <div className="rounded-xl border border-slate-200 bg-white p-6">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            {customer.address && <p className="text-sm text-slate-500">{customer.address}{customer.city ? `, ${customer.city}` : ""}</p>}
      ...
      <Section title={`Purchases (${purchases.length})`}>
        ...
        {purchases.map((p) => (
          ...
          >
            <span>
              <span className="font-mono text-xs font-medium text-indigo-700">{p.invoiceNo}</span>
              <span className="ml-2 text-sm font-medium">{p.vehicleDesc ?? "—"}</span>
              <span className="mt-0.5 block text-xs text-slate-400">{d(p.saleDate)}</span>
            </span>
      ...
      <Section title={`Bookings (${bookings.length})`}>
        ...
        {bookings.map((b) => (
          <div key={b.id} className="flex items-center justify-between rounded-lg border border-slate-200 bg-white px-4 py-2.5 text-sm">
            ...
            <span className="flex items-center gap-3">
              ...
              <span className="text-xs text-slate-400">{d(b.createdAt)}</span>
            </span>
          </div>
        ))}
      </Section>
      ...
      <Section title={`Test Drives (${rides.length})`}>
        ...
        {rides.map((r) => (
          <div key={r.id} className="flex items-center justify-between rounded-lg border border-slate-200 bg-white px-4 py-2.5 text-sm">
      ...
      <Section title={`Workshop Visits (${jobs.length})`}>
        ...
        {jobs.map((j) => (
          <Link
            key={j.id}
            href={`/workshop/${j.id}`}
            className="flex items-center justify-between rounded-lg border border-slate-200 bg-white px-4 py-2.5 text-sm hover:border-indigo-300"
          >
            <span>
              <span className="font-mono text-xs font-medium text-indigo-700">{j.jobNo}</span>
              <span className="ml-2 font-mono text-xs text-slate-400">{j.chassisNo}</span>
            </span>
            <span className="flex items-center gap-3">
              ...
              <span className="text-xs text-slate-400">{d(j.createdAt)}</span>
            </span>
          </Link>
        ))}
      </Section>
...
function Empty({ text }: { text: string }) {
  return <p className="rounded-lg border border-slate-200 bg-white px-4 py-4 text-sm text-slate-400">{text}</p>;
}
